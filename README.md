...
8. [Q-Digest 自适应数据融合](#8-q-digest-自适应数据融合)
9. [终端服务器：Random Forest坑洞判断](#9-终端服务器random-forest坑洞判断)
10. [可视化输出](#10-可视化输出)
11. [文件结构](#11-文件结构)
12. [技术栈](#12-技术栈)
...
| 数据融合 | Q-Digest 近似分位数摘要（自适应阈值 + 分布统计） |
| 数据处理 | 加权平均数据融合 |
...
传感器原始数据（CSV）
  ...
  Cluster Head M/D/1 队列
        ↓
  ┌─ Q-Digest 累积更新 ─────────────┐
  │  插入 mag_max、az_std 到分位摘要  │
  └─────────────────────────────────┘
        ↓
  CH 自适应预过滤（Q-Digest 阈值）
        ↓ 通过
  加权数据融合 + 窗口 Q-Digest 统计
  （Q25/Q50/Q75/Q90 作为元数据附加）
        ↓
...
## 7. Cluster Head：M/D/1排队 + 预过滤 + 数据融合（含 Q-Digest）
...
### 7.2 自适应粗过滤（边缘预处理，Q-Digest 驱动）

CH 转发前进行轻量级坑洞预判断，过滤明显正常路面数据，减少上行传输量。

**早期阶段（< 30 个样本）**：使用固定阈值（与原始设计相同）：

```python
mag_max > 1.2  or  az_std > 0.15
```

**自适应阶段（≥ 30 个样本）**：阈值由 Q-Digest 分布自动推导：

```python
thresh_mag = max(Q60(mag_max历史分布), 1.2)
thresh_az  = max(Q60(az_std历史分布),  0.15)
```

- Q60 代表历史 60% 分位数——道路越颠簸，Q60 越高，阈值自动上调，仅保留真正异常的峰值
- `max(Q60, 固定值)` 保证阈值**永远不低于**原始值，维持高召回率
- 适应不同城市路段的加速度基线，减少"虚警"流量
...
**Q-Digest**（Shrivastava et al., SenSys 2004）是专为传感器网络设计的**近似分位数摘要**数据结构，支持在资源受限节点上以 O(k log U) 空间维护全局分布统计。

核心思想：在整数域 `[0, U)` 上构建二叉树，每个节点 `(lo, hi)` 存储落在该区间的计数；通过**自底向上压缩**（将小计数节点合并至父节点）控制树的大小，同时保证每个分位数查询的秩误差 ≤ n/k。
...
| 类 | 说明 |
|----|------|
| `QDigest(U, k)` | 整数域 `[0, U)` 的核心摘要；`k` 控制精度与空间的权衡 |
| `ScalarQDigest(v_min, v_max, U, k)` | 浮点封装，将 `[v_min, v_max]` 线性映射到整数 bin |
...
|------|------|
| `insert(v)` | 向叶节点插入一个值 |
| `compress()` | 自底向上压缩：若 `count(v) + count(parent) + count(sibling) ≤ n/k` 则合并 |
| `merge(other)` | 节点级相加（C₁₂(I) = C₁(I) + C₂(I)），再压缩 |
| `quantile(q)` | 按左端点遍历分区，累计计数，返回首次超过 q·n 的区间中点 |

#### 集成到 Cluster Head

```
每次成功接收数据包：
  → 将 mag_max（特征[8]）插入 ScalarQDigest[0,20 g]
  → 将 az_std （特征[5]）插入 ScalarQDigest[0,5 g]
  → 每 50 包执行一次 compress()

每个融合窗口（3 s）：
  → 对窗口内所有通过预过滤的包构建 per-window Q-Digest
  → 计算 Q25 / Q50 / Q75 / Q90 并作为元数据附加到融合批次
  → 输出格式：{"qdigest": {"mag_max_q50": ..., "az_std_q90": ...}}
```
...
| 参数 | 值 | 说明 |
|------|----|------|
| `QD_K` | 20 | 压缩因子；秩误差上界 = n/20 |
| `QD_U` | 128 | 离散化 bin 数；精度 ≈ 范围/127 |
| `QD_MAG_MAX_RANGE` | [0, 20] g | mag_max 的值域上界 |
| `QD_AZ_STD_RANGE` | [0, 5] g | az_std 的值域上界 |
| `QD_MIN_SAMPLES` | 30 | 自适应阈值激活所需最少样本数 |
| `ADAPTIVE_QUANTILE` | 0.60 | 使用 Q60 作为自适应阈值基准 |

### 8.4 输出示例

仿真摘要中新增 Q-Digest 阈值表：

```
CH   n_samp  adaptive  mag_thresh  az_thresh  mag_Q50  mag_Q90   az_Q50   az_Q90
CH0     662       yes      1.2598     0.1500   1.2598   1.4173   0.0787   0.1575
CH1    1717       yes      1.2000     0.1500   1.1024   1.4173   0.0787   0.1575
CH2      19        no      1.2000     0.1500      —        —        —        —
```

`wsn_results.json` 中同时记录所有 CH 的 `qdigest_filter_stats`（含各分位数值）。

---

## 9. 终端服务器：Random Forest坑洞判断
...
│   ├── cluster_head.py         # CH（TDMA + M/D/1 + Q-Digest 自适应预过滤 + 融合）
│   ├── base_station.py         # 终端服务器（RF 模型推理）
│   └── qdigest.py              # Q-Digest 数据结构（QDigest + ScalarQDigest）
│
...
├── test_qdigest.py             # Q-Digest 单元测试（37 项，含端到端集成测试）
│
...
| 数据融合 | Q-Digest（自实现，Shrivastava et al. 2004）|
